...
def load_lastfm():
    path = 'data_raw/lastfm/'
    user_info_path = "{}/user_info.csv".format(path)
    item_info_path = "{}/item_info.csv".format(path)
    rating_path = '{}/user_artists_ratings.csv'.format(path)

    user_info = pd.read_csv(user_info_path,
                            names=['user_id', 'friend'],
                            sep=",", engine='python')
    item_info = pd.read_csv(item_info_path,
                            names=['item_id', 'category'],
...
def dict_storing_lastfm():
    ...
    for i_id in tqdm(item_all_ids):
        ...
        if len(row) > 0:
            feature_vector = item_converting_lf(item_row=row, category_list=list_lastfm['list_category'])
