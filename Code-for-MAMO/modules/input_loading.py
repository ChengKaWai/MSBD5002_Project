...
class BKItemLoading(torch.nn.Module):
    ...
    def forward(self, x2):
        ...
        return concat_emb
#lastfm
class LFUserLoading(torch.nn.Module): # sharing
    def __init__(self, embedding_dim):
        super(LFUserLoading, self).__init__()
        self.friend_dim = config['n_friend']
        self.embedding_dim = embedding_dim

        self.embedding_friend = torch.nn.Linear(in_features=self.friend_dim, out_features=self.embedding_dim, bias=False)
    def forward(self, x1):
        friend_idx = x1[:,0:1892]
        friend_emb = F.sigmoid(self.embedding_friend(friend_idx.float()))
        return friend_emb

#lastfm
class LFItemLoading(torch.nn.Module): # sharing
    def __init__(self, embedding_dim):
        super(LFItemLoading, self).__init__()
        self.category_dim = config['n_category']
        self.embedding_dim = embedding_dim

        self.embedding_category = torch.nn.Linear(in_features=self.category_dim, out_features=self.embedding_dim, bias=False)
